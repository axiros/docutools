...
function setup_termcasts(window, document) {
  ...
  function fetch_term_json(url, callback) {
    fetch(url)
      ...
      .then((json) => callback(null, json, url))
      .catch((error) => callback(error, null, url));
  }
  ...
  function linkify_callflow(svg) {
     ...
     *   b. for the details data
     * 3) we generate evt. handlers on all hrefs within the svg
     * 4) we open the details window (W)
     * 5) add hilite css into it
     * 6) add hilite js to our window
     * 7) when loaded hljs, we call load_details_json, which loads the details data json
     * 8) we put the data into us, in element details_win_transfer_content
     * 9) and juse hiliter we just loaded to add col css
    ...
    by_tag_name("footer")[0].innerHTML += `
      ...
      <div id="details_win_transfer_content" style="visibility: hidden">
       <div style="margin: 20px; overflow: auto">
        ...
        <h5 class="pth"></h5>
        <h2 >Input</h2>
            <pre><code class="json input"> </code></pre>
        <hr></hr>
        <h2 >Function</h2>
            <pre style="background-color: #333"><code class="python func"></code></pre>
        <h2 >Output</h2>
